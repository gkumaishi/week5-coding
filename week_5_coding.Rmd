...
# QQ-Plot compares our sample to data if it were perfectly drawn from a normal dist, the closer to linear our QQ plot gets, the closer it is to normal distribution
...
```{r}
t.test(chinstraps_m, chinstraps_f, alternative = "greater") # Is male mass *greater* than female mass? Alternative should "fit in the middle"
```
...
### 2. Find summary statistics and check for equal variance
...
leveneTest(flipper_length_mm ~ species, data = penguin_f) #leveneTest in "cars" pkg
```
